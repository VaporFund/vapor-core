...
// Simple Marketplace for trading Pandora ERC-404

contract PandoraMarket is ERC721Holder {
    ...
    // add liquidity as ERC-20
    function addLiquidity(uint256 _tokenAmount) external {

        Pandora(tokenAddress).safeTransferFrom(
            msg.sender,
            address(this),
            _tokenAmount
        );

        liquidity += _tokenAmount;
    }


    // get the ERC-721 token
    function buy(uint256 unit) external {
        Pandora(tokenAddress).transfer(
            msg.sender,
